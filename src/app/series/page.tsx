...
export default async function SeriesPage({ searchParams }: { searchParams: Promise<{ sort?: string }> }) {
  const resolvedParams = await searchParams
  const sort = resolvedParams?.sort === 'release' ? 'release' : 'timeline'

  const series = await prisma.title.findMany({
    where: { 
      type: { in: ['SERIES', 'SPECIAL', 'SHORT'] } 
    },
    orderBy: sort === 'release' ? { year: 'asc' } : { timelineOrder: 'asc' },
  })

  // Group by phase (timeline) or decade (release)
  let grouped: Record<string, typeof series> = {}
  
  if (sort === 'release') {
    grouped = series.reduce((acc, title) => {
  ...
  const sortedKeys = Object.keys(grouped).sort((a, b) => {
    if (sort === 'release') return a.localeCompare(b)
    ...
    <div className="min-h-screen bg-[#040714] text-white pt-24 pb-20">
      <main className="px-6 md:px-16 mx-auto max-w-[1600px]">
        <header className="mb-12 max-w-2xl">
          <h1 className="font-heading text-4xl md:text-5xl font-black tracking-tight mb-4 text-white drop-shadow-md">Catálogo de Séries</h1>
          <p className="text-lg text-zinc-400 font-medium mb-6">
            Mergulhe fundo nas histórias episódicas do MCU.
          </p>

          <div className="flex bg-zinc-900/50 p-1 rounded-lg w-fit border border-white/5">
            <Link 
              href="/series?sort=timeline"
